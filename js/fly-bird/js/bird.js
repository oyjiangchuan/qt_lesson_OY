var bird = {
    ...
    showBird: function(parentObj) {
        ...
        parentObj.appendChild(this.div) // 把小鸟div插入到游戏页面中
    },
    fallSpeed: 0, // 小鸟下落的速度
    ...
    flyBird: function() { // 控制小鸟下落的函数
        ...
        function fly() {
            bird.div.style.top = bird.div.offsetTop + bird.fallSpeed++ + 'px' // 小鸟匀速下落
            if (bird.div.offsetTop < 0) {
                bird.fallSpeed = 2 // 这里用于控制小鸟不能飞出顶部
            }
            if (bird.div.offsetTop >= 395) {
                bird.fallSpeed = 0
                clearInterval(bird.flyTimer) // 一旦飞到地面清除,取消定时器
                clearInterval(bird.wingTimer)
            }
            if (bird.fallSpeed > 10) {
                bird.fallSpeed = 10 // 控制小鸟下落速度最大为10
            }
        }
    },
